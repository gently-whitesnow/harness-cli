...
/// <summary>
/// A file on disk but not in the Git index is invisible to the harness; these tests pin the
/// diagnostic that tells "never written" from "written and never staged".
/// </summary>
public sealed class UntrackedEvidenceTests
{
    ...
    /// <summary>`harness init` leaves the frame unstaged and the run incomplete without a finding,
    /// so only the check's own declaration can explain it.</summary>
    [Fact]
    ...
    /// <summary>"Not applicable" hides the same trap: a project nobody staged looks exactly like
    /// a repository with no .NET projects at all.</summary>
    [Fact]
    ...
    /// <summary>The declaration is required of every check and printed, so a missing file cannot
    /// be reported silently.</summary>
    [Fact]
