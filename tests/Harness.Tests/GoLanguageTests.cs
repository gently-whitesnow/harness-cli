...
public sealed class GoLanguageTests
{
    ...
    public void Doc_comments_directives_raw_strings_generated_vendor_and_testdata_do_not_count()
    {
        ...
        using var repository = Fixtures.Compliant(Frame.AllPresent().Generated(
                """[{"paths":["internal/gen"],"reason":"stringer output"}]"""))
    ...
    public void Nolint_without_linters_or_without_a_reason_is_blocking_and_a_justified_one_passes()
    {
        ...
        Assert.Equal(1, run.ExitCode);
        Assert.True(run.OutputContains("internal/sample/run.go:4: silences errcheck at one address via //nolint"), run.Output);
        ...
        Assert.True(run.OutputContains("run.go:7"), run.Output);
    ...
    public void Prose_and_justified_specific_nolint_directives_still_pass(string comment)
    {
        ...
        Assert.Equal(comment.StartsWith("//nolint:", StringComparison.Ordinal) ? 1 : 0, run.ExitCode);
    }
    ...
    public void Toml_and_json_configurations_are_read_by_tables_and_by_the_bcl()
    {
        ...
        Assert.True(tomlRun.OutputContains("linters.disable-all switches off every linter"), tomlRun.Output);
    ...
    public void Explain_describes_each_go_check_in_its_own_terms()
    {
        ...
        Assert.True(suppressions.OutputContains("Every `//nolint` directive blocks")
            && suppressions.OutputContains("settings.lint-suppressions.go.repositoryWide"), suppressions.Output);
    }
