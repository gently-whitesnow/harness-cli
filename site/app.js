...
(function () {
  ...
  const CHECKS = [
    ...
    { id: 'complexity.csharp', group: 'arch', axis: 'csharp', settings: 'complexity.csharp', summary: 'DSM по продукту: внутри архитектурных зон, а без зоны — вне tracked тестовых проектов. Потолок объявлен в settings.complexity.csharp: meanReach и coreSize, дефолт контракта 8.0 файла и 0 из sliced-dotnet/1; превышение блокирует, отчёт называет файлы вне Host с наибольшей достижимостью.', adr: ['0032-topology-over-thresholds.md', '0042-dsm-over-the-product-in-files.md', '0048-dsm-product-boundary-without-a-zone.md', '0052-dsm-ceiling-is-a-declared-setting.md'] },
  ...
  const GROUPS = [
    ...
    { key: 'arch', title: 'Архитектура и DSM', note: 'Первый и второй ярусы контракта 2.0. Секция architecture выбирает стандарт sliced-dotnet/1 либо applicable: false; DSM сравнивается с потолком из settings.complexity.csharp, дефолт 8.0 файла mean reach и ядро 0.' },
  ...
  const DSM_LIMIT = { meanReach: 8, core: 0 }; // defaults of settings.complexity.csharp
  const dsm = { svg: $('#dsm-svg'), nodes: [], edges: [], leaves: 0, cycle: false, selected: null };
  function resetDsm() {
  ...
  function renderDsm() {
    ...
    $('#dsm-limit-reach').textContent = fmt(DSM_LIMIT.meanReach);
    $('#dsm-limit-core').textContent = String(DSM_LIMIT.core);
    const reachEl = $('#dsm-reach'), coreEl = $('#dsm-core');
    const reachExceeded = m.meanReach > DSM_LIMIT.meanReach + 1e-9, coreExceeded = m.core > DSM_LIMIT.core;
    reachEl.className = 'stat__value ' + (reachExceeded ? 'is-error' : '');
    coreEl.className = 'stat__value ' + (coreExceeded ? 'is-error' : '');
    ...
    if (reachExceeded || coreExceeded) {
      finding.className = 'finding';
      const lines = ['❌ complexity.csharp'];
      if (reachExceeded) lines.push(`   src: mean reach ${fmt(m.meanReach)} files exceeds the ${fmt(DSM_LIMIT.meanReach)} files the standard allows; cut edges from the hubs named below or change the tracked policy knowingly.`);
      if (coreExceeded) for (const i of m.coreNodes) lines.push(`   ${dsm.nodes[i].name}  This file belongs to the largest SCC (${m.core} files); the standard allows ${DSM_LIMIT.core} — break the cycle.`);
      finding.textContent = lines.join('\n') + selectedLine;
    } else {
      finding.className = 'finding is-ok';
      finding.textContent = `✅ complexity.csharp  limits: mean reach ${fmt(DSM_LIMIT.meanReach)} files · core size ${DSM_LIMIT.core} files · mean reach: ${fmt(m.meanReach)} files (${m.pairs} reachable file pairs / ${m.n} files; propagation cost ${fmt(m.cost)}%) · core size: ${m.core} files · scope: ${m.n} files inside architecture zone [src]${selectedLine}`;
    }
  }
  ...
  function renderArch() {
    ...
    slice('Orders', 32, 150, 132);
    slice('Billing', 176, 150, 132);
  ...
  function defaultState(profileKey) {
    ...
    const s = {
      ...
      settings: {
        ...
        complexity: { meanReach: 8, coreSize: 0 },
  ...
  function buildConfig() {
    ...
    settings['complexity.csharp'] = { meanReach: state.settings.complexity.meanReach, coreSize: state.settings.complexity.coreSize };
  ...
  function renderBuilder() {
    ...
    for (const group of GROUPS) {
      ...
      const list = el('div', { class: 'field-list' }, checks.map((check) => {
        ...
        return el('div', { class: 'field' + (na ? ' is-off' : '') }, [
          el('div', { class: 'field__label' }, [el('code', { text: check.id }), el('small', { text: na ? 'ось не применима → NotApplicable при любой policy; запись всё равно обязательна' : '' })]),
    ...
    root.appendChild(step(5, 'Policy каждой проверки', '<code>required</code> блокирует, <code>advisory</code> показывает находки без провала, <code>off</code> пропускает. Переключатель един, включая инвариант архитектуры и DSM-пределы; адресных исключений нет.', policyBody));
    ...
    const settingsBody = el('div', { class: 'field-list' }, [
      ...
      el('div', { class: 'field' + (state.axes.csharp.applicable ? '' : ' is-off') }, [
        el('div', { class: 'field__label' }, [el('code', { text: 'settings.complexity.csharp' }), el('small', { text: 'потолок DSM: meanReach в файлах (не меньше 1) и coreSize; дефолт 8.0 / 0 — значения sliced-dotnet/1' })]),
        el('div', { class: 'inline-fields' }, [numField('meanReach', state.settings.complexity.meanReach, (v) => { state.settings.complexity.meanReach = Math.max(1, v); }), numField('coreSize', state.settings.complexity.coreSize, (v) => { state.settings.complexity.coreSize = v; })]),
      ]),
      el('div', { class: 'field' }, [
  ...
  function renderOutput() {
    ...
    tally.appendChild(el('span', { class: 'badge badge--success badge--mono', text: 'settings 6/6' }));
