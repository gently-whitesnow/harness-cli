...
(function () {
  ...
  /* ── Check catalogue (execution order of CheckRegistry, contract 2.10.0) ── */
  const CHECKS = [
  ...
  function buildConfig() {
    ...
    return {
      version: state.latest ? 'latest' : '2.10.0',
  ...
  function evaluate(config) {
    ...
    add('ok', `version "${config.version}" — контракт ${config.version === 'latest' ? 'следует за установленным бинарём' : '2.10.0, тот же, что исполняет бинарь'}.`);
  ...
  function renderBuilder() {
    ...
    const archBody = el('div', { class: 'field-list' }, [
      ...
      el('div', { class: 'field' }, [
        el('div', { class: 'field__label' }, [el('code', { text: 'version' }), el('small', { text: '"2.10.0" пинит контракт; "latest" включает rolling-контракт вслед за установленным бинарём.' })]),
        seg([{ value: 'pin', label: '2.10.0' }, { value: 'latest', label: 'latest' }], state.latest ? 'latest' : 'pin', (v) => { state.latest = v === 'latest'; update(); }),
      ]),
    ]);
