...
(function () {
  ...
  /* ── Check catalogue (execution order of CheckRegistry, contract 2.9.0) ── */
  const CHECKS = [
    ...
    { id: 'complexity.csharp', group: 'arch', axis: 'csharp', budget: true, summary: 'DSM по файлам внутри архитектурных зон: mean reach и core size против tracked .harness.budget.json. Превышение потолка блокирует; harness budget update только ужимает.', adr: ['0032-topology-over-thresholds.md', '0042-dsm-over-the-product-in-files.md'] },
    ...
    { id: 'frame.tests.unit', group: 'frame', key: 'tests.unit', summary: 'Ответ репозитория про unit-тесты: где они, или почему их нет.', hint: 'tests/Unit' },
    { id: 'frame.tests.integration', group: 'frame', key: 'tests.integration', summary: 'Ответ про интеграционные тесты; границу между видами тестов определяет сам репозиторий.', hint: 'tests/Integration' },
  ...
  const GROUPS = [
    ...
    { key: 'frame', title: 'Рамка: self-reported вопросы', note: 'Одна и та же анкета для всех. Формы ответа: { "paths": [...] } · { "present": true|false, "reason": "..." } · { "applicable": false, "reason": "..." }. present: false — readiness gap, а не провал; harness init оставляет ответы пустыми и ставит off, кроме verify.' },
  ...
  function buildConfig() {
    ...
    return {
      version: state.latest ? 'latest' : '2.9.0',
  ...
  function evaluate(config) {
    ...
    add('ok', `version "${config.version}" — контракт ${config.version === 'latest' ? 'следует за установленным бинарём' : '2.9.0, тот же, что исполняет бинарь'}.`);
  ...
  function renderBuilder() {
    ...
    const archBody = el('div', { class: 'field-list' }, [
      ...
      el('div', { class: 'field' }, [
        el('div', { class: 'field__label' }, [el('code', { text: 'version' }), el('small', { text: '"2.9.0" пинит контракт; "latest" включает rolling-контракт вслед за установленным бинарём.' })]),
        seg([{ value: 'pin', label: '2.9.0' }, { value: 'latest', label: 'latest' }], state.latest ? 'latest' : 'pin', (v) => { state.latest = v === 'latest'; update(); }),
      ]),
    ]);
