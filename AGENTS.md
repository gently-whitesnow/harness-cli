...
Каждая применимая проверка априори `required`. `policy` нужен для явного усиления до `strict`,
смягчения до `advisory` или отключения через `off`; `strict` делает blocking даже
эвристические находки, которые сама проверка оставляет advisory. `suppress` — именованные
...
[ADR-0017](adrs/0017-required-by-default.md), [ADR-0027](adrs/0027-strict-repository-policy.md)
