...
`overrides` перенастраивает норму path-scoped проверки для glob-зоны: запись называет
`check`, `paths` (`*`, `**`, `?`), обязательный `reason` и либо частичные `settings`, либо
`"off": true`, исключающий зону из анализа. Suppress принимает находку, override меняет
норму до измерения; секция входит в контракт релиза `1.1.0`.
[ADR-0024](adrs/0024-path-scoped-overrides.md)
