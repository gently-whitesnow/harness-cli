...
Каждая применимая проверка априори `required`: любая enforceable-находка blocking, даже если
эвристика помечает исходную уверенность как advisory. Наблюдения дают scale и не повышаются
policy; `policy` смягчает enforceable-находки до `advisory` или отключает проверку через
`off`, `suppress` принимает именованное исключение с `reason`. Поэтому `present: false` по
умолчанию — нарушение. Харнес валидирует полноту ответов, но не ищет опровержения.
...
Связанность и связность считаются по tracked-исходникам средствами BCL. Ребро графа несёт
`Proven` (позиция допускает только тип, кандидат по имени один) или `Inferred`. Blocking
строится только из `Proven`, и такая находка одна — цикл модулей. Начиная с 1.5 fan-in/out
считают только `Proven` и остаются observations при любой policy; `--all` печатает полный
список. Вложенный модуль — содержание, а не цикл. [ADR-0021](adrs/0021-coupling-evidence-grades.md),
[ADR-0029](adrs/0029-dependency-observations.md)
