...
дочерние конфиги не содержат `version`, `projects` или настройки commit-интеграции.
Секция `settings` обязательна даже пустая: `"settings": {}`.
Наследования нет: у каждого проекта явная рамка. Например, проект с Markdown-данными
может задать `"docs.policy": "off"` или `"advisory"` в своей `policy`, сохранив
`required` у других проектов. Исключения для отдельных файлов и находок не поддерживаются.

Корень проверяет файлы вне проектов; каждый проект — свой каталог. Ответы и локальные
пути читаются относительно компонента. Общие tracked `.editorconfig` и `Directory.*`
выше каталога проекта доступны как настройки, но не расширяют измерение на соседний код.
Корневые документы не заменяют обязательные документы проекта при включённой `docs.policy`.

`harness check` из любого каталога репозитория проверяет весь workspace.
`harness check --project apps/api` выполняет частичный прогон: сначала валидирует все
конфиги workspace, затем проверяет корневую область и выбранный проект. Соседние проекты
не измеряются. `--verbose` раскрывает область измерений. В CI используйте полный прогон.
