...
для библиотеки — `architecture.applicable: false`. Все проверки — `required`; смягчение обсуждается с владельцем.
...
- Ansible: полный digest, секреты и форма ролей (required из init), причины inline `noqa`, циклы ролей;
