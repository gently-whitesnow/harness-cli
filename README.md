...
`HARNESS_VERSION=1.6.0` ставит конкретный релиз, `HARNESS_INSTALL_DIR` меняет каталог
...
- связанность: граф доказанных зависимостей между модулями; цикл модулей блокирует прогон;
- связность: типы, распадающиеся на группы членов, которые не разделяют состояние;
- maintainability hotspots — размер файлов, типов и методов и лексическое ветвление;
