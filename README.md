...
Все применимые проверки по умолчанию `required`; `policy: strict` осознанно делает любую
находку выбранной проверки blocking, а `advisory`/`off` смягчают или отключают её.
`suppress` хранит именованные исключения с обязательной причиной.
