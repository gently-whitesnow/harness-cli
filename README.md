...
`HARNESS_VERSION=2.8.0` ставит конкретный релиз, `HARNESS_INSTALL_DIR` меняет каталог
...
```yaml
harness:
  image: ghcr.io/gently-whitesnow/harness:2.8.0
  script:
    - harness check
...
строку DSM-бюджета рядом с текущими mean reach/core size и областью измерения. Модель трёх ярусов,
