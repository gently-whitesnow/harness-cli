...
отдельно: `harness check --only <check-id> [--verbose]`. Если файл, который проверка искала,
лежит в рабочем дереве без `git add`, отчёт называет его строкой `not in the index`:
untracked-файл не является доказательством, но и молчать о нём незачем. Команда
