...
2. Backend-расширение добавляет `Common`, `Manager`, `Managers`, `Repository` и
   ...
   потребителе скрыли несколько назначений в одном bucket. В коде этот набор хранится
   отдельно от буквального порта `BAD_NAMES_GENERIC`, чтобы локальная политика не выдавалась за
   правило Steiger.
