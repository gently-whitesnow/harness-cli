...
- [ADR-0064](0064-typescript-language-axis.md) — контракт 3.6: TS/JS импорты, дубли и DSM с прозрачными barrels; 8.0 / 0.
...
- [ADR-0017](0017-required-by-default.md) — Исторический required-default; контракт 2.0
  требовал полный явный `policy` для каждого shipped check — правило полноты заменено ADR-0054.
...
- [ADR-0065](0065-hook-resolves-the-harness-at-commit-time.md) — Managed `commit-msg` не
...
- [ADR-0066](0066-adr-shape-as-decision-record.md) — `adrs.shape` требует форму записи
  решения, непрерывные уникальные номера, статус, дату и объявленные в рамке лимиты.
...
- [ADR-0051](0051-slices-in-the-layer-root.md) — Слайсы лежат в корне слоя `<Слой>/<Слайс>`
  без `Features/`; корень слайсового слоя содержит только слайсы, `Contracts/` в нём —
  `no-layer-public-api`, группа-обёртка и сегмент-эхо — advisory; контракт `2.13.0`, имя
  стандарта остаётся `sliced-dotnet/1` — версия в имени как механизм отменена, переход
  несёт pin.
...
- [ADR-0032](0032-topology-over-thresholds.md) — Контракт 2.0 удаляет
  ...
  Ярус 2 (ratchet-бюджет) заменён ADR-0052.
...
- [ADR-0042](0042-dsm-over-the-product-in-files.md) — `complexity.csharp` измеряет файлы
  внутри архитектурных зон sliced-dotnet и считает среднюю достижимость файлов
  вместо propagation cost; маркер `<auto-generated>` в области измерения называется в
  отчёте. Бюджетная часть заменена ADR-0052.
...
- [ADR-0052](0052-dsm-ceiling-is-a-declared-setting.md) — Потолок DSM объявляется в
  `settings."complexity.csharp"` (`averageReachableFiles`, `largestCyclicGroupSize`) с дефолтом контракта 8.0 / 0 из
