...
- [ADR-0027](0027-required-findings-are-blocking.md) — `required` превращает каждую
  оставшуюся находку проверки в blocking-нарушение; `advisory` даёт явное время на
  исправление, а suppress применяется до оценки policy.
...
- [ADR-0029](0029-dependency-observations.md) — Начиная с 1.5 dependency fan-in/fan-out
  считают только `Proven`-рёбра и вместе с external imports остаются observations;
  module cycles по-прежнему blocking, `--all` раскрывает полный измеренный список.
